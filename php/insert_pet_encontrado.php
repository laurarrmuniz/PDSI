...
if (
    ...
    try {
        ...
        $sql = "INSERT INTO pet_encontrado (local, titulo, data, horario, info, tipo, porte, sexo, id_usuario) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $sql2 = "INSERT INTO foto_pets_encontrado (id_pet_encontrado, nomeArqFoto) VALUES (?, ?)";
        ...
        if ($stmt1->execute()) {
            $id_pet_encontrado = $conn->conexao->lastInsertId(); //id do último pet_encontrado cadastrado

            foreach ($fotos as $nomeArqFoto) {
                $stmt2 = $conn->conexao->prepare($sql2);
                $stmt2->bindParam(1, $id_pet_encontrado);
