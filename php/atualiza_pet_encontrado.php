...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["id"]) && !empty($_POST["id"])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE pet_encontrado SET titulo = ?, local = ?, data = ?, horario = ?, info = ?, tipo = ?, porte = ?, sexo = ? WHERE id = ?");
            ...
            if (!empty($_FILES['foto']['name'][0])) {
                foreach ($_FILES['foto']['tmp_name'] as $key => $tmp_name) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO foto_pets_encontrado (id_pet_encontrado, nomeArqFoto) VALUES (?, ?)");
